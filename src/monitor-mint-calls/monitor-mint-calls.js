...
const { monitorMintCalls, umaEverestId } = require('../../agent-config.json');

// set up a timeout to allow the JSON-RPC endpoint to respond with trace data
const { rpcTimeoutMilliseconds } = monitorMintCalls;
...
function provideHandleBlock(provider) {
  return async function handleBlock(blockEvent) {
    ...
    let traces = await provider.send('trace_block', params);

    // the Alchemy JSON-RPC endpoint does not always respond with trace data for the current block
    while ((traces === null) && ((Date.now() - start.getTime()) < rpcTimeoutMilliseconds)) {
      // eslint-disable-next-line no-await-in-loop
      traces = await provider.send('trace_block', params);
    }
