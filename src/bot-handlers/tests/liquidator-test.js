...
describe("liquidator-test.js", function () {
  let accounts;
  // Roles
  let sponsor1;
  let sponsor2;
      ...
      describe(`${testConfig.collateralDecimals} collateral, ${testConfig.syntheticDecimals} synthetic & ${testConfig.priceFeedDecimals} pricefeed decimals, for smart contract version ${contractVersion.contractType} @ ${contractVersion.contractVersion}`, function () {
        ...
        versionedIt([{ contractType: "any", contractVersion: "any" }])(
          ...
          async function () {
            ...
            financialContractClient.getAllPositions().filter((position) =>
              // both sponsor 1 and sponsor 2 should be liquidatable
              position.sponsor === sponsor1 || position.sponsor === sponsor2
            ).forEach((position) => {
